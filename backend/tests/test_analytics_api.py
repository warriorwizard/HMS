...
async def _request(
    ...
    *,
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        return await client.request(method=method, url=path, params=params, headers=headers)
...
def test_analytics_events_filters_and_pagination_contract() -> None:
    _install_test_db()
    ...
    assert [item["id"] for item in payload["items"]] == ["evt_005"]
    assert payload["items"][0]["event_type"] == "ai_summary_generated"


def test_tat_metrics_filters_and_pagination_contract() -> None:
    _install_test_db()
    token = _login_and_get_access_token()
    ...
    response = asyncio.run(
        _request(
            ...
            "/api/v1/analytics/tat-metrics",
            token=token,
            params={"status": "stable", "department": "radiology,cardiology", "limit": 1, "offset": 1},
        )
    )
    ...
    assert set(payload["page"]) == {"limit", "offset", "total"}
    assert payload["page"] == {"limit": 1, "offset": 1, "total": 2}
    assert [item["id"] for item in payload["items"]] == ["tat_003"]
    assert payload["items"][0]["department"] == "cardiology"
