...
async function PartnerDirectoryPanel({ filters, dataPromise }: PartnerDirectoryPanelProps) {
  ...
  try {
    ...
    return <PartnerDirectoryFallback filters={filters} message={b2bMessage(error)} />;
  }
...
async function PricingRulesPanel({ filters, dataPromise }: PricingRulesPanelProps) {
  ...
  try {
    ...
    return <PricingRulesFallback message={b2bMessage(error)} />;
  }
...
function PartnerDirectoryFallback({
  filters,
  message
}: {
  filters: PartnerFilters;
  ...
  const partners = [
    {
      id: "PAR-APEX",
      name: "Apex Hospitals Network",
      code: "APEX-NORTH",
      contact: "Priya Nair",
      contactMeta: "priya@apex.example | +91 90000 11001",
      status: "active",
      segment: "Hospital network",
      location: "Delhi, NCR"
    },
    {
      id: "PAR-NOVA",
      ...
      code: "NOVA-LABS",
      contact: "Rahul Shah",
      contactMeta: "rahul@nova.example | +91 90000 11002",
      status: "pending",
    ...
    {
      id: "PAR-SOUTH",
      name: "SouthZone Clinic Chain",
      code: "SOUTH-CLINIC",
      contact: "Amina Khan",
      contactMeta: "amina@southzone.example | +91 90000 11003",
      status: "active",
      segment: "Clinic chain",
  ...
  const filteredPartners = partners.filter((partner) => {
    const matchesQuery =
      query.length === 0 ||
      [partner.id, partner.name, partner.code, partner.contact, partner.segment, partner.location]
        .join(" ")
        .toLowerCase()
        .includes(query);
    const matchesStatus = status.length === 0 || partner.status.includes(status);
    return matchesQuery && matchesStatus;
  });

  return (
    <Panel className="wide-panel" meta={`${filteredPartners.length} partners`} title="Partner Table">
      <FieldHint tone="warning">{message} Showing partner operating directory.</FieldHint>
      <div className="data-table compact-top">
        <div className="table-row table-head">
          <span>Partner</span>
          <span>Primary contact</span>
        ...
        {filteredPartners.length > 0 ? (
          filteredPartners.map((partner) => (
            <div className="table-row" key={partner.id}>
              <div>
                <strong>{partner.name}</strong>
                <small>{partner.code} | {partner.id}</small>
              </div>
              <div>
                <strong>{partner.contact}</strong>
                <small>{partner.contactMeta}</small>
              </div>
              <StatusBadge tone={statusTone(partner.status)}>{partner.status}</StatusBadge>
              <div>
                <strong>{partner.segment}</strong>
          ...
          <EmptyState
            title="No partners found"
            description="No demo partners match the current query or status filter."
          />
        )}
      </div>
    </Panel>
...
function PricingRulesFallback({ message }: { message: string }) {
  const rules = [
    ["Apex radiology priority", "PAR-APEX", "INR 1,050/study", "active"],
    ["Nova pathology volume slab", "PAR-NOVA", "18.00% discount", "pending"],
    ["SouthZone clinic package", "PAR-SOUTH", "INR 720/order", "active"]
  ];

  return (
    <Panel meta={`${rules.length} rules`} title="Pricing Rules Snapshot">
      <FieldHint tone="warning">{message} Showing partner pricing snapshot.</FieldHint>
      <div className="key-value-list compact-top">
        ...
        <div>
          <span>Active rules</span>
          <strong>{rules.filter((rule) => rule[3] === "active").length}</strong>
        </div>
      </div>
      ...
      <div className="data-table compact-top">
        ...
        {rules.map(([name, partner, rate, status]) => (
          <div className="table-row" key={name}>
            <div>
              <strong>{name}</strong>
              <small>Contracted partner pricing</small>
            </div>
            <span>{partner}</span>
            <span>{rate}</span>
...
function b2bMessage(error: unknown): string {
  return isApiError(error)
    ? `The B2B API responded with an error: ${error.message}.`
    : "The B2B API is currently unavailable.";
}
